...
export function sourceStatus(file: ProjectFile): string {
  ...
  const incomplete = (d.pageStatus || []).filter(p => !['native', 'blank'].includes(p.status));
  return `${d.stale ? 'Refresh failed · using previous text' : d.state === 'failed' ? 'Not readable' : d.state === 'partial' ? 'Partially readable' : 'Native text ready'}${pages}` +
