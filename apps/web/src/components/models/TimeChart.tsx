...
export function TimeChart({ title, series, times, max, unit, digits = 1, height = 132, empty }: {
  ...
  return <figure className="viz-chart" aria-label={title}>
    ...
    <div ref={box} className="viz-plot">
      {n < 2 ? <p className="viz-empty">{empty || 'Collecting samples…'}</p> : <>
        <svg width={width} height={height} role="img" aria-label={`${title}, last ${Math.round(span / 60)} minutes`} onPointerMove={onMove} onPointerLeave={() => setHover(null)}>
          ...
          <text className="viz-axis" x={pad.l} y={height - 4}>{Math.round(span / 60)} min ago</text>
          <text className="viz-axis" x={pad.l + w} y={height - 4} textAnchor="end">now</text>
        ...
        {hover != null && <div className="viz-tip" style={{ left: Math.min(width - 150, Math.max(0, x(hover) - 70)) }}>
          <span>{Math.round(times[n - 1] - times[hover])} s ago</span>
