...
type Step = { kind: 'spec' | 'prompt'; id: string; label: string; status: string; reason?: string; score?: number; promptPerSecond?: number; seconds?: number;
...
export function AutoTune({ model, onChanged }: { model: string; onChanged: () => void }): JSX.Element {
  ...
  const [confirmed, setConfirmed] = useState(false), [extend, setExtend] = useState(true), [busy, setBusy] = useState(false), [error, setError] = useState('');
  const done = useRef('');
  const refresh = async () => {
  ...
  const start = async () => {
    ...
    try {
      const r = await apiFetch('/api/models/autotune', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ model, confirmPause: confirmed, extendContext: extend }) });
    ...
    {mine && <div aria-live="polite">
      ...
      {mine.steps.length > 0 && <div className="mm-table-wrap" role="region" aria-label="Auto-tune steps" tabIndex={0}><table className="mm-table">
        ...
        <tbody>{mine.steps.map((s, i) => <tr key={i}>
          <td>{s.label}</td>
          <td>{s.status === 'measured' ? 'measured' : s.reason || s.status}</td>
      ...
      {mine.result && <div className="mm-easy-result" role="status"><div className="mm-easy-result-text">
        ...
        {mine.result.extensions.map((e) => <p key={e.id} className="mm-note">{e.id === 'context' ? `Context can likely grow from ${(e.from || 0).toLocaleString('en-US')} to about ${(e.to || 0).toLocaleString('en-US')} tokens: ${e.why}.` : `Suggested: ${e.why}.`}</p>)}
