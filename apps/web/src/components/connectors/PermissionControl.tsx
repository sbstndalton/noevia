...
export const MODE_LABEL: Record<ToolMode, string> = { allow: 'Always allow', ask: 'Needs approval', block: 'Blocked' };
...
export function PermissionControl({ tool, value, write, busy, onChange }: {
  ...
  return <div ref={track} className="glass-seg permission-control" role="radiogroup" aria-label={`${tool} permission`} aria-busy={busy || undefined} onKeyDown={onKey}>
    {MODES.map(([mode, icon]) => {
      const locked = write && mode === 'allow';
      const label = locked ? 'Always allow (writes always ask first)' : MODE_LABEL[mode];
