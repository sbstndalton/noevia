...
export function PalettePicker({ theme, onChange, bare = false }: { theme: 'light' | 'dark'; onChange?: (palette: Palette) => void; bare?: boolean }) {
  ...
  // `bare`: the surrounding settings row already names and explains the control.
  return <fieldset className={`palette-picker${bare ? ' is-bare' : ''}`} aria-label={bare ? `Color palette for ${theme} mode` : undefined}>
    {!bare && <legend>Color palette</legend>}
    {!bare && <p>Choose the palette for {theme} mode. Your other mode keeps its own choice.</p>}
