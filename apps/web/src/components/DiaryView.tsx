...
import { diaryRequest, directoryPicker, listFiles, readFile, saveLocal, scanLocal, syncFileChange, writeFile } from '../diary-workspace';
...
export function DiaryView({ inferenceUp }: { inferenceUp?: boolean | null }) {
  ...
  const composer = <div className="diary-compose">
    ...
    <button className="send-btn" aria-label="Send diary message" disabled={busy || !draft.trim()} onClick={submit}>↑</button></div>
  ...
  return <main className="main diary-workspace">
    ...
    <div className="diary-layout"><section className="diary-primary">
      ...
      {!month && <div className="diary-landing"><p className="section-eyebrow">A little space for yourself</p><h1>How has your day been?</h1><p className="diary-intro">A moment, a thought, a question. Start wherever you are.</p>{composer}</div>}
