  ...
  return <>
    ...
    {features && <div className="set-rows feature-settings">{features.filter(f => !!f.experimental === experimental).map(f => <div className="set-row set-row-inline" key={f.name}>
      <div className="set-row-text">
        ...
        <span className="set-row-desc" role="status">{busy === f.name ? 'Saving…' : f.unavailable || (f.experimental ? (f.enabled ? 'On · experimental routing selected' : 'Off · current routing selected') : '')}</span>
