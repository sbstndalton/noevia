...
export function LocalGraph({ path, text, root, busy, onSearch, onOpen }: Props) {
  ...
  useEffect(() => {
    ...
    search.current('', path, controller.signal, 'backlinks')
      ...
      .catch((e) => { if (abort.current === controller) setState({ status: 'error', backlinks: [], partial: false, error: controller.signal.aborted ? 'The scan stopped before finishing.' : e instanceof Error ? e.message : 'The scan failed.' }); })
  ...
  return (
    <details className="diary-local-graph" onToggle={(e) => setOpen((e.currentTarget as HTMLDetailsElement).open)}>
      ...
      <p>This file, the notes it links to, and the notes that link back. Links back use the same bounded scan as backlinks.</p>
      {state.status === 'loading' && <p role="status">Finding links to this file…</p>}
      {state.status === 'error' && <p role="alert">{state.error} <button type="button" className="popup-tab" onClick={() => setAttempt((n) => n + 1)}>Retry</button></p>}
      {open && (
        <figure className="diary-local-graph-figure">
          <svg viewBox={`${-R - 22} ${-R - 22} ${2 * R + 44} ${2 * R + 44}`} role="group" aria-label={`Local graph of ${graph.nodes[0].label}`}>
            ...
            {graph.nodes.map((n) => {
              ...
              return (
                <g key={n.id} className={`graph-node graph-node-${n.relation}`} transform={`translate(${n.x * R} ${n.y * R})`}
                  ...
                  aria-label={self ? undefined : `Open ${n.id} (${n.relation === 'both' ? 'links both ways' : n.relation === 'out' ? 'linked from this file' : 'links to this file'})`}
          ...
          <figcaption>
            <span className="graph-key graph-key-out">Links out {(counts.out || 0) + (counts.both || 0)}</span>
            <span className="graph-key graph-key-in">Links in {(counts.in || 0) + (counts.both || 0)}</span>
            {graph.hidden > 0 && <span>{graph.hidden} more not shown</span>}
            {state.partial && <span>Partial: not every file was scanned</span>}
          </figcaption>
          {groups.length > 0 && <div className="graph-list">
            {groups.map((g) => <section key={g.relation} aria-label={g.relation === 'both' ? 'Links both ways' : g.relation === 'out' ? 'Links out' : 'Links in'}>
              <h3>{g.relation === 'both' ? 'Both ways' : g.relation === 'out' ? 'Links out' : 'Links in'} ({g.nodes.length})</h3>
