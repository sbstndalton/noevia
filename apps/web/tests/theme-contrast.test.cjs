...
for (const palette of ['cool','warm','neutral','sage','iris']) for (const mode of ['dark','light']) test(`${palette} ${mode}: semantic text, actions and control contrast`, () => {
  ...
  if (palette !== 'cool') {
    const override = css.match(new RegExp(`\\[data-theme='${mode}'\\]\\[data-palette='${palette}'\\] \\{([^}]+)`))[1];
    Object.assign(tokens, Object.fromEntries([...override.matchAll(/--([\w-]+):\s*(#[\da-f]{6})/gi)].map(m=>[m[1],m[2]])));
  }
