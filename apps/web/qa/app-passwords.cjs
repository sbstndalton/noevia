...
(async()=>{
 ...
 try{
  ...
  await member.reload();
  await member.getByRole('button',{name:/Account menu for/}).click();await member.locator('.account-popover').getByRole('menuitem',{name:'Settings',exact:true}).click();
  ...
  await member.reload();await member.getByRole('button',{name:/Account menu for/}).click();await member.locator('.account-popover').getByRole('menuitem',{name:'Settings',exact:true}).click();await member.getByRole('button',{name:'Security and login',exact:true}).click();
