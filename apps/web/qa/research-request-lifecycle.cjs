...
(async () => {
  ...
  try {
    ...
    let bJobs = [], failNextBGet = false, heldGet = null;
    ...
    const stale = { started:deferred(), release:deferred(), jobs:[job()] };
    heldGet = stale;
    ...
    await page.getByRole('alert').getByText('Research refresh failed').waitFor();
    await page.close();
    console.log('PASS research request lifecycle: project reset, stale plan/poll, and successful start/cancel/save snapshots after refresh failures');
