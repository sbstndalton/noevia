...
(async()=>{
 ...
 const upstream=http.createServer(async(req,res)=>{
  ...
  if(req.url==='/metrics'){res.setHeader('Content-Type','text/plain');return res.end(!metricsAvailable?'':'lemonade_llamacpp_spec_decode_num_draft_tokens_total{model_name="Synthetic native"} 100\nlemonade_llamacpp_spec_decode_num_accepted_tokens_total{model_name="Synthetic native"} 75\n');}
  if(req.url.endsWith('/chat/completions')){
   res.setHeader('Content-Type','text/event-stream');
   if(body.diary_events)return res.end('data: '+JSON.stringify({type:'mtp',model:'Synthetic native',timings:{draft_n:40,draft_n_accepted:20}})+'\n\ndata: {"type":"answer","text":"Synthetic diary answer"}\n\ndata: {"type":"diary","decision":"skip"}\n\ndata: {"type":"done"}\n\n');
   return res.end('data: '+JSON.stringify({model:'Synthetic native',timings:{draft_n:40,draft_n_accepted:32},choices:[{delta:{content:'Synthetic chat answer'},finish_reason:'stop'}]})+'\n\ndata: [DONE]\n\n');
  }
 ...
 try{
  ...
  const page=await browser.newPage();page.on('pageerror',e=>console.error('BROWSER',e));await page.goto(origin);
  ...
  const project=(await api(page,'/api/projects',{name:'Synthetic MTP',model:'Synthetic native',toolboxes:[]})).body;
  assert.equal((await api(page,'/api/chat',{spaceId:'project',projectId:project.id,message:'Synthetic telemetry test',history:[]})).status,200);
  sample=(await api(page,'/api/stats')).body.mtp[0];assert.equal(sample.rate,.8);assert.equal(sample.source,'last response');
