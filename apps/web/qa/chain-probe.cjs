// Diagnostic: walk up Diary ancestors, report heights/overflow.
...
(async()=>{
  ...
  const chrome=spawn('/Applications/Google Chrome.app/Contents/MacOS/Google Chrome',['--headless=new','--remote-debugging-port=0','--user-data-dir=/tmp/noevia-probe3','--no-first-run','about:blank'],{stdio:['ignore','pipe','pipe']});
  ...
  const report=await ev(`(()=>{
    const chain=[];
    let el=document.querySelector('.diary-composer-dock');
    while(el&&el!==document.documentElement){
      const cs=getComputedStyle(el);
      chain.push({cls:el.className.slice(0,50),h:Math.round(el.getBoundingClientRect().height),minH:cs.minHeight,maxH:cs.maxHeight,disp:cs.display,ovf:cs.overflow});
      el=el.parentElement;
    }
    // composer internals
    const dock=document.querySelector('.diary-composer-dock');
    const kids=[...dock.querySelectorAll('*')].slice(0,40).map(k=>({cls:k.className&&String(k.className).slice(0,40)||k.tagName,h:Math.round(k.getBoundingClientRect().height),top:Math.round(k.getBoundingClientRect().top)}));
    return {chain,kids};
  })()`);
