...
await navClick(p,'Projects');await p.reload();await p.getByRole('textbox',{name:'Message',exact:true}).waitFor();assert.equal(historyReads,0);assert.equal(f.requests.length,0);
