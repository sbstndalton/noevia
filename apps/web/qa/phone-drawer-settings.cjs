...
(async()=>{
 ...
 for(const theme of ['light','dark']){
  ...
  assert.equal(m.fabRadius,'16px');assert.ok(parseFloat(m.active)>=20,`${theme} M3: active destination is a pill`);assert.equal(m.composerRadius,'28px');assert.doesNotMatch(m.font,/^Roboto/,'Material 3 inherits the shared UI font (fafbc97)');
  await page.close();
 }
