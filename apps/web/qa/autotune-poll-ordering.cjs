...
(async()=>{
 ...
 const bundled=await build({stdin:{contents:`import React,{useState} from 'react';import{createRoot}from'react-dom/client';import{AutoTune}from'./src/components/models/AutoTune';function Harness(){const[model,setModel]=useState('Model A'),[shown,setShown]=useState(true);return <main style={{padding:24,maxWidth:1000,margin:'auto'}}><button onClick={()=>setModel('Model B')}>Select B</button><button onClick={()=>setShown(false)}>Unmount</button>{shown&&<AutoTune model={model} onChanged={()=>{window.qaChanges=(window.qaChanges||0)+1;}}/>}</main>}createRoot(document.getElementById('root')).render(<Harness/>);`,loader:'tsx',resolveDir:root},bundle:true,write:false,format:'iife',jsx:'automatic',define:{'process.env.NODE_ENV':'"production"'}});
