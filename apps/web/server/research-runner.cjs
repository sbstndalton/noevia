...
function createResearchRunner({ jobs, search, extract, projectRetrieve = async () => [], complete, now = Date.now, options = {} }) {
  ...
  async function start({ question, projectId = null, subQuestions = null }) {
    ...
    const done = jobs.run(id, async (ctx) => {
      ...
      const parts = [];
      let total = 0, valid = 0;
      for (const [i, sub] of (subQuestions?.length ? subQuestions : [q]).entries()) {
        if (ctx.signal.aborted) break;
        if (now() > budget.deadline) { ctx.progress('Time budget reached'); break; }
        ctx.progress(`Researching ${i + 1}: ${sub}`);
        const capped = await gather(sub, ctx, registry, budget);
        const drafted = await section(sub, capped, ctx);
      ...
      const body = parts.map((p) => (parts.length > 1 ? `## ${p.question}\n\n` : '') + p.markdown).join('\n\n');
      const markdown = `# ${q}\n\n${body}\n\n## Sources\n\n${rs.sourcesFooter(registry) || '_None._'}\n`;
      return { markdown, sources: registry.list(), citationValidity: total ? valid / total : 1, citations: total, webCalls: budget.webCalls, partial: ctx.signal.aborted };
    });
    return { id, done };
  }
