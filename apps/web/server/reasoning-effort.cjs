...
function modeFor(provider, model, effort) {
  return effort === 'default' ? 'off' : documented(provider, model) && !rejected.has(capabilityKey(provider, model)) ? 'real' : 'hint';
}
function requestBody(body, effort, mode, provider) {
  if (mode === 'off') return body;
  if (mode === 'real') return { ...body, reasoning_effort: effort };
...
async function requestWithEffort(fetcher, url, options, body, provider, model, effort, report) {
  ...
  if (response.status >= 400 && response.status < 500) {
    ...
    if (mode === 'real' && /reasoning_effort/i.test(detail)) {
...
module.exports = { validEffort, resolveEffort, modeFor, requestBody, requestWithEffort };
