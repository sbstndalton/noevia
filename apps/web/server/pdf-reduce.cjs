...
// The largest honest answer is a base64 PDF at OUTPUT_CAP plus a small JSON envelope; anything
// bigger is refused while streaming, never buffered whole (#130/#139).
const RESPONSE_CAP=Math.ceil(OUTPUT_CAP/3)*4+64*1024;
async function readCappedJson(response,limit=RESPONSE_CAP){
 const tooLarge=()=>Object.assign(Error('Document processor response was too large.'),{status:502});
 const declared=Number(response.headers?.get?.('content-length'));
 if(Number.isFinite(declared)&&declared>limit){response.body?.cancel?.().catch(()=>{});throw tooLarge();}
 let text;
 if(response.body&&typeof response.body.getReader==='function'){
  const reader=response.body.getReader(),chunks=[];let received=0;
  for(;;){const {done,value}=await reader.read();if(done)break;received+=value.length;if(received>limit){reader.cancel().catch(()=>{});throw tooLarge();}chunks.push(Buffer.from(value));}
...
async function prepare(name,bytes,{progress=()=>{},url=process.env.OCR_BASE_URL,fetchImpl=fetch}={}){
 ...
 const result=await readCappedJson(response);let output,outputName;
...
module.exports={prepare,INPUT_CAP,RESPONSE_CAP,readCappedJson};
