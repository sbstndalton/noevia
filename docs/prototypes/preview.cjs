...
const settings=require('./settings-fixture.cjs').createSettingsFixture();
...
if(await settings(req,res,url))return;
if(req.method==='GET'&&/^\/api\/projects\/[^/]+\/instruction-skills$/.test(url.pathname))return json({skills:[]});
if(url.pathname==='/api/workspace')return json({projects,freeChats:[]});
const projectChats=url.pathname.match(/^\/api\/projects\/([^/]+)\/chats$/);
if(req.method==='GET'&&projectChats)return json({chats:projects.find(p=>p.id===projectChats[1])?.chats||[]});
if(req.method==='GET'&&/^\/api\/chats\/[^/]+\/history$/.test(url.pathname))return json({history:[{role:'user',content:'Summarise what is verified and what still needs a decision.'},{role:'assistant',content:'The restore check is complete. The remaining decision is the client access workflow.\n\nKeep the verified configuration in place until that workflow has been tested. This is a synthetic conversation for the interface preview.'}]});
