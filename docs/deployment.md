...
first (gzip-verified); `RELEASE_a73a72d_COMPLETE`, five services healthy, restarts=0. Server
tests 823/823, QA 74/74. Front end only.
Rollback: repoint `current` to `releases/d99c066`, restore `.env.bak.before-a73a72d`, run
preflight `up.sh … web diary ocr`.
...
## Release f9dc5df — 2026-09-21 (Auto routing default)

Overlay `67f336e` → `f9dc5df` after appdata backup `ab_20260921_171118` (all archives `gzip -t` clean); no compose or engine changes. All containers healthy with 0 restarts; site 200. Retain `67f336e`.
